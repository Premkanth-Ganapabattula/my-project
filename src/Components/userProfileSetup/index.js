...
class UserProfileSetup extends Component {
    state = {redirectToHome: false, username: '', profilePhoto: noProfilePhoto, userFirstName:'', bio: '', relationshipStatus: ''}
    ...
    getFirstName = async() => {
        ...
        const url = `http://localhost:8080/profile/upload/${userId}`
        const options = {
            method: 'GET',
            headers: {
        ...
        this.setState({userFirstName: data.userDetails.first_name, username: data.userDetails.first_name + " " + data.userDetails.last_name})
    }
    ...
    onSubmitDetails = async event => {
        ...
        userAdditionalDetails.append('profilePhoto', profilePhoto); // Assuming profilePhoto is a File object
        ...
        const url = "http://localhost:8080/user/additional/details"
        const options = {
            method: "POST",
            headers: {
                Authorization: `Bearer ${jwtToken}` 
    ...
    render() {
        const {userFirstName, profilePhoto, redirectToHome} = this.state
        if (redirectToHome) {
            return <Redirect to="/" />
        }
        return(
            <div className='user-profile-setup-container'>
                <form className='content-container' encType="multipart/form-data" onSubmit={this.onSubmitDetails}>
                    ...
                    <div className='save-button-card'>
                        <button className='save-button-element' type="submit">Save</button>
                    </div>
                </form>
            </div>
        )
